 ...
 */
public class ClimateFromStations_ConstantStations {
    
    public ClimateFromStations_ConstantStations(String pcaDataLocation, String stationDataLocation) throws Exception {
        // Load station data
        ...
        for (int month = 0; month <= 12; month++) {
            ...
            // Get the "common" stations we want to focus on
            List<Long> commonStations = new ArrayList<Long>();
            {
                ...
                if (month == 0) {
                    ...
                    date = dateFormat.parse("1900-" + StringUtils.leftPad(Integer.toString(month), 2, "0") + "-01");
                }
                
                List<StationResults> stations = stationData.GetDate(month, date);
                for (int i = 0; i < stations.size(); i++) {
                    StationResults currStation = stations.get(i);
                    commonStations.add(currStation.StationId);
                }
            }
            ...
            for (Date currDate : monthDates) {
                ...
                stations = stations.stream()
                        .filter(a -> commonStations.contains(a.StationId))
                        .sorted((a,b) -> Long.compare(b.StationId, a.StationId))
                        .collect(Collectors.toList());
                    ...
                    for (int l = 0; l <= k; l++) {
                        ...
                        Callable<Quartet<Integer, Integer, Complex, double[]>> callable = new Callable<Quartet<Integer, Integer, Complex, double[]>>() {
                            public Quartet<Integer, Integer, Complex, double[]> call() throws Exception {
                                ...
                                // Now that we have the weights for given k,l, calculate the 
                ...
                for (Future<Quartet<Integer, Integer, Complex, double[]>> future : futures) {
                    ...
                    finalStationHarmonic.SetHarmonic(k, l, future.get().getValue2());
                }
                
                //ComplexArray.Print(finalStationHarmonic.GetFullSpectral());
    ...
    public static void main(String args[]) throws Exception {   
        ClimateFromStations_InputParser in = new ClimateFromStations_InputParser(args, ClimateFromStations_ConstantStations.class.getName());
        if (in.InputsCorrect()) {
            ClimateFromStations_ConstantStations c = new ClimateFromStations_ConstantStations(
