 ...
 */
public class ClimateFromStations_Global_WithVariance {
    
    public ClimateFromStations_Global_WithVariance(String pcaDataLocation, String stationDataLocation, double varExpCutoff, boolean normalized, String output) throws Exception {
        // Load station data
        ...
        // Load the variance object
        Map<Pair<Integer, Integer>, double[][]> variances = (Map<Pair<Integer, Integer>, double[][]>) LoadSerializedObject("Results/Variance/Dataset=air.sfc.mon.mean.nc/Years=30/variance.serialized");
            ...
            for (Date currDate : monthDates) {
                ...
                Callable<Triplet<Date, Double, double[]>> callable = new Callable<Triplet<Date, Double, double[]>>() {
                    public Triplet<Date, Double, double[]> call() throws Exception {
                        ...
                        Pair<Integer, Integer> key = Pair.with(currDate_f.getYear() + 1900, month_f);
                        double[][] gridBoxAnomSd = DoubleArray.Power(variances.get(key), 0.5);
                        ...
                        if (uniformWeight) {
                            ...
                            Triplet<Integer, Integer, List<Long>> weightKey = Triplet.with(0, 0, stationList);
                            //if (false) {
                            if (weightsMap.containsKey(weightKey)) {
                                weights = weightsMap.get(weightKey);                                   
                            } else {
                                Complex[] stationHarmonics_k_l = stationHarmonics.GetHarmonic(0, 0);
                                Complex[] stationHarmonics_k_l_conj = stationHarmonics.Conjugate().GetHarmonic(0, 0);
                                ...
                                for (int e = 0; e < numEigen; e++) {
                                    ...
                                    // C Nx1
                                    Complex Psi_lm = currEigenSpherical.Conjugate().GetHarmonic(0, 0);
                                    double oscil = Math.pow(-1, 0);
                                ...
                                if (solver.isNonSingular()) {
                                    ...
                                    System.out.println("Date: " + currDate + ", stationCount: " + stationId.length + ", k: " + 0  + ", l:" + 0);
                                }
                            }
                        }
                        ...
                        finalStationHarmonic.SetHarmonic(0, 0, S_lm);
    ...
    public static void main(String args[]) throws Exception {   
        ClimateFromStations_InputParser in = new ClimateFromStations_InputParser(args, ClimateFromStations_Global_WithVariance.class.getName());
        if (in.InputsCorrect()) {
            ClimateFromStations_Global_WithVariance c = new ClimateFromStations_Global_WithVariance(
                    in.dataEof, in.dataStations, in.varExplained, in.normalized, in.output);
        }
    }
}
