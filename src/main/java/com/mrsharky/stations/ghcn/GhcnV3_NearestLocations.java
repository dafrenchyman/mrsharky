 ...
 * @author jpierret
 */
public class GhcnV3_NearestLocations extends HolePunchSelection {
    ...
    public GhcnV3_NearestLocations(String monthlyData, String inventoryData, QcType qcType, double[] lats, double[] lons
    ...
    protected List<StationResults> GetStations(List<Row> StationData) {
        ...
        List<Triplet<Long, Double, Double>> FinalStationIds = new ArrayList<Triplet<Long, Double, Double>>();
        ...
        if (StationData.size() > 0) {
            ...
            for (int i = 0; i < _lats.length; i++) {
                ...
                FinalStationIds.add(Triplet.with(bestStationId, lat1, lon1));
            }
            ...
            for (int j = 0; j < StationData.size(); j++) {
                ...
                stationResultsLookup.put(stationId, currStation);
            }

            for (int i = 0; i < FinalStationIds.size(); i++) {
                Triplet<Long, Double, Double> stationData = FinalStationIds.get(i);

                long stationId = stationData.getValue0();
                double lat = stationData.getValue1();
                double lon = stationData.getValue2();
                StationResults currStation = stationResultsLookup.get(stationId);

                StationResults newStation = new StationResults();
                newStation.Cnt = currStation.Cnt;
                newStation.Lat = lat;
                newStation.Lon = lon;
                newStation.BaselineMean = currStation.BaselineMean;
                newStation.StationId = stationId;
                newStation.Value = currStation.Value;
                newStation.BaselineVariance = currStation.BaselineVariance;

                finalStations.add(newStation);
            }
        }
        if (_verbose) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (false) {
            ...
            AreasForGrid areasForGrid = new AreasForGrid(10, 20,1.0);
            lats = areasForGrid.GetLatitude();
            lons = areasForGrid.GetLongitude();
        }
        
        GhcnV3_InputParser parser = new GhcnV3_InputParser(args, GhcnV3_NearestLocations.class.getName());
        if (parser.InputsCorrect()) {
            GhcnV3_NearestLocations ghcn = new GhcnV3_NearestLocations(parser.monthlyData, parser.inventoryData, parser.qcType, lats, lons, parser.minDistance, 
