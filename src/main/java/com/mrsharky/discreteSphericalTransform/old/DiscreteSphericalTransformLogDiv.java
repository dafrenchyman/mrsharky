 ...
 * @author dafre
 */
public class DiscreteSphericalTransformLogDiv {
    ...
    public DiscreteSphericalTransformLogDiv(double[][] DATA, int Q, boolean GAUSQUADon) throws Exception {
        int M = DATA.length+1;
        int N = DATA[0].length;
        ...
        for (int k = 0; k < Q+1; k++) {
            ...
            FieldMatrix<Complex> oscYnegL = MatrixUtilities.ToComplexFieldMatrix(MatrixUtilities.ElementwiseMatrixPower(MatrixUtilities.Tile(DoubleArray.ToRealMatrix(new double[]{-1}),1,k+1)
                    , l.transpose()));
            oscYnegL = MatrixUtilities.ElementwiseMultiplication(oscYnegL.transpose(), left);
            ...
            // Sectra
            FieldMatrix<Complex> oscSpectraYpos = MatrixUtilities.Tile(MatrixUtilities.ToComplexFieldMatrix(MatrixUtilities.ElementwiseMatrixPower(MatrixUtilities.Tile(DoubleArray.ToRealMatrix(new double[]{-1}),1,k+1)
                                        , l.transpose())).transpose(),1,N);
            FieldMatrix<Complex> oscSpectraYneg = MatrixUtilities.Tile(MatrixUtilities.ToComplexFieldMatrix(MatrixUtilities.ElementwiseMatrixPower(MatrixUtilities.Tile(DoubleArray.ToRealMatrix(new double[]{-1}),1,k+1)
                                        , l.scalarMultiply(-1.0).transpose())).transpose(),1,N);
            
            double[] phi2 = legZerosRad.getColumn(0);
            for (int i = 0; i < phi2.length; i++) {
                phi2[i] = Math.cos(phi2[i]);
            }
            
            RealMatrix legendreMatrix = (new AssociatedLegendrePolynomials(k,phi2)).GetAsRealMatrix();
            ...
            for (int counterM = 1; counterM < M; counterM++) {
                ...
                //RealMatrix legTemp = legendreMatrix.getColumnMatrix(counterM-1);
                 
                // Ys
                FieldMatrix<Complex> middle = MatrixUtilities.ToComplexFieldMatrix(MatrixUtilities.Tile(legTemp,1,N));
                FieldMatrix<Complex> YposL = MatrixUtilities.ElementwiseMultiplication(MatrixUtilities.ElementwiseMultiplicationTile(
                            left, middle),
                        exponentialYposL);            
                FieldMatrix<Complex> YnegL = MatrixUtilities.ElementwiseMultiplication(MatrixUtilities.ElementwiseMultiplicationTile(
                            oscYnegL, middle),
                        exponentialYnegL);
                ...
                // Spectra Building
                {
                    FieldMatrix<Complex> middleSpectra1 = DATA_comp.getRowMatrix(counterM-1);
                    
                    // Spectra Building Positive
                    FieldMatrix<Complex> spectraLeft = MatrixUtilities.ElementwiseMultiplication(MatrixUtilities.ElementwiseMultiplicationTile(oscSpectraYpos, middleSpectra1),
                    ...
                    FieldMatrix<Complex> spectraRight = MatrixUtilities.ElementwiseMultiplication(MatrixUtilities.ElementwiseMultiplicationTile(oscSpectraYneg, middleSpectra1),
