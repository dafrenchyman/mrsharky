...
import com.mrsharky.discreteSphericalTransform.*;
 ...
 */
public class AssociatedLegendrePolynomials_ass_old {
    ...
    public double AssLegFunc2(int l, int m, double x) {
        ...
        if (alreadyCalculated) {
            ...
            if (false) {
                ...
                for (int i = 1; i <= m; i++) {
                    value *= right*doubleFactorial / (doubleFactorial+1.0);
                    doubleFactorial += 2.0;
                }
            ...
            if (false) {
                ...
                value = x*Math.sqrt(2.0 * m + 3.0) * AssLegFunc2(m,m,x);
            }
            this._calculatedValues.get(x).get(l).put(m, value);
            return value;
        } else {
            double value = 0.0;
            if (false) {
                value = (x*(2*l -1) * AssLegFunc2(l-1,m,x) - (l+m-1)*AssLegFunc2(l-2,m,x))/(l-m);
            } else {
                ...
                value = Math.sqrt(leftTop/leftBot) * (x*AssLegFunc2(l-1,m, x) - Math.sqrt(rightTop/rightBot) * AssLegFunc2(l-2, m,x));
            }
            this._calculatedValues.get(x).get(l).put(m, value);
            return value;
        }
    ...
    public AssociatedLegendrePolynomials_ass_old(int N, double X) {
    ...
    public AssociatedLegendrePolynomials_ass_old(int K, double[] X) {
        this._calculatedValues = new HashMap<Double, Map<Integer,Map<Integer,Double>>>();
    ...
    public static void main(String args[]) throws Exception {
        ...
        AssociatedLegendrePolynomials_ass_old leg = new AssociatedLegendrePolynomials_ass_old(10,x);
