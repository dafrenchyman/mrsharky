...
public class Shtns {
    ...
    private native double[] getLats(long cfg);
    private native double[] getLons(long cfg);
    ...
    */
    ...
    static {
        try {
            ...
            addLibraryPath("/media/dropbox/PhD/Reboot/Projects/Ncep20thCenturyReanalysisV2c/C/shtns_jni/shtns_jni/dist/Debug/GNU-Linux");
    ...
    public Shtns(int lmax, int mmax, int mres, int nlat, int nphi, int numThreads) throws Exception {
        ...
        // Calculate the lon values
        int latTotal = _mres*_nphi;
        _lons = DoubleArray.Multiply(linspace(1.0,latTotal,latTotal), 2*Math.PI/latTotal);
    }
    
    // Getters
    public double[] GetLats() { return _lats; }
    public double[] GetLons() { return _lons; }
    ...
    public static void main(String[] args) throws Exception { 
        ...
        if (true) {
            spectral[5] = new Complex(1.0, 0.0);
            shtns.SpectralToSpatial(spectral);
            double[][] newDataset = shtns.GetSpatial();
            
            shtns.SpatialToSpectral(newDataset);
            Complex[] newSpectral = shtns.GetSpectralCompressed();
            ComplexArray.Print(newSpectral);
        }
        ...
        DoubleArray.Print(latCoordinates);
        ...
        for (int i = 0; i < rebuilt.length; i++) {
            for (int j = 0; j < rebuilt[0].length; j++) {
                double currError = Math.pow(data[i][j] - rebuilt[i][j], 2.0);
                Error[i][j] = currError;
                double currErrorSin = currError * Math.sin(latCoordinates[i]);
                error += currErrorSin;
            }
        ...
        double spectraError = shtns.GetSpectralCompressed()[0].getReal();
